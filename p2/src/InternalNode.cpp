...
void InternalNode::lazyRemove(int index)
{
...
BTreeNode* InternalNode::rotateFromLeft(int value)
{
    InternalNode *sibling = (InternalNode *)leftSibling;

    int index = leftSibling->getCount() - 1;
    //int parentReplaceKey = sibling->keys[index];
    BTreeNode *parentReplace = sibling->children[index];

    sibling->lazyRemove(index);
    insert(parentReplace);
    parent->resetMinimum(this);
    /*
    ...
    {
        if(parentNode->keys[i] > parentReplaceKey)
        {
            insert(parentNode->children[i]);
            parentNode->keys[i] = parentReplaceKey;
            parentNode->children[i] = parentReplace;
        }
    }
    */
    return NULL;
...
BTreeNode* InternalNode::rotateFromRight(int value)
{
...
void InternalNode::removeFromChild(int value)
{
    int pos; // will be where value belongs
    for(pos = count - 1; pos > 0 && keys[pos] > value; pos--);
    BTreeNode *child = children[pos];

    BTreeNode *mergedNode = child->remove(value);
    if(!mergedNode)  // child had room.
    {
        return;
    }

    //put the key in the mergedNode
    //mergedNode->insert(keys[pos]);

    //set correct siblings
    if(child->getLeftSibling())
    ...
    //delete the fucked up child
    delete child;

    //shift the arrays and shit
    int i;
...
BTreeNode* InternalNode::merge(BTreeNode *other)
{
    int i;
    //we are doing this check because
    //the insert function is flawed
    //so we need to do the insert in a specific order

    if(other->getMinimum() < getMinimum()) //to the left, insert in order
    {
        for(i = 0; i < count; i++)
        {
            ((InternalNode *)other)->insert(children[i]);
        }
    }
    else //to the right, insert in reverse order
    {
        for(i = count - 1; i >= 0; i--)
        {
            ((InternalNode *)other)->insert(children[i]);
...
BTreeNode* InternalNode::getParent()
{
    return parent;
}
...
{  // to be written by students
    ...
    removeFromChild(value);

    int minSize = (internalSize + 1) / 2;
    ...
    else if(leftSibling)
    {
        if(leftSibling->getCount() > minSize)
            result = rotateFromLeft(value);
        else
            result = merge(leftSibling);
    }
    else if(rightSibling)
    {
        if(rightSibling->getCount() > minSize)
            result = rotateFromRight(value);
        else
        {
            result = merge(rightSibling);
        }
    }
    ...
    if(result || parent)
        ...
        return (count == 1 ? children[0] : NULL);
