         ...
         */
        template <class HashedObj>
        void QuadraticHashTable<HashedObj>::insert( const HashedObj & x, void *userData )
        {
                // Insert x as active
            int currentPos = findPos( x );
            if( isActive( currentPos ) )
                return;
            array[ currentPos ] = HashEntry( x, ACTIVE, userData );
        ...
        void QuadraticHashTable<HashedObj>::rehash( )
        {
            ...
            for( int i = 0; i < oldArray.size( ); i++ )
                if( oldArray[ i ].info == ACTIVE )
                    insert( oldArray[ i ].element, oldArray[ i ].userData );
        }
...
/* 7*/      return currentPos;
         ...
         */
        template <class HashedObj>
        void * QuadraticHashTable<HashedObj>::find( const HashedObj & x ) const
        {
            int currentPos = findPos( x );
            return isActive( currentPos ) ? array[ currentPos ].userData : NULL ;
        }
